...
fout <- file.path(prj_path, "data/intermediate/compiled_potato_fertiliser_trial_calculated_supply_afterlmer_sqrttf.RDS")
	...
	out <- lapply(tlid, function(i) {
	  ...
	  #at least 3 rows of data are needed + attainable yield:
		if (length(unique(dsi$treat)) > 2 & !is.na(Yai)){
			...
			si <- Rquefts::revSupply(dsi, qcrop, qsoil, Yai, leaf_ratio=0.17, stem_ratio=0.14, SeasonLength = 120)
